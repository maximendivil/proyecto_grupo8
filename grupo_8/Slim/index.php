...
require_once "../modelo/modelo_funciones.php";
...
// GET route
$app->get('/cuotasPagas/:dni/:year', function ($dni,$year) {
    try {
        $cuotasPagas = buscarCuotasPagasAPI($dni,$year);
        header('Access-Control-Allow-Origin: *'); 
        echo json_encode($cuotasPagas);
    }
    catch (Exception $e){
        header('Access-Control-Allow-Origin: *'); 
...
$app->get('/cuotasImpagas/:dni/:year', function ($dni,$year) {
    try {
        
        $cuotasImpagas = buscarCuotasImpagasAPI($dni,$year);
        header('Access-Control-Allow-Origin: *'); 
        echo json_encode($cuotasImpagas);
    }
    catch (Exception $e){
        header('Access-Control-Allow-Origin: *'); 
        echo json_encode($e->getMessage());
...
$app->get('/ingresos/:year', function ($year) {
    try {
        $ingresosDelMes = buscarIngresosAPI($year);
        header('Access-Control-Allow-Origin: *'); 
        echo json_encode($ingresosDelMes);
    }
    catch (Exception $e){
        header('Access-Control-Allow-Origin: *'); 
        echo json_encode($e->getMessage());
