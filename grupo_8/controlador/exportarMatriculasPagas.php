	...
	function exportarMatriculas(){
		...
		if (rolConsulta($_SESSION["rol"])){
			try{
				...
				$rows = obtenerAlumnosConMatriculaPagaDeUnResponsable($retorno[0]["id"],0,$total);//
		...
		//Para el rol de administracion o de gestion, muestro todos los alumnos
			...
			try{
				...
				$rows = alumnosConMatriculaPaga(0,$total);//
				//$total_paginas = ceil($total / $paginacion);
		...
		require("exportarAlumnosConMatriculaPaga.php");
	}
